        ...
        if($searchFor != "")
        {
            ...
            $devices = $devs->join('labs','devices.lab_id','labs.id')
             ...
             $devs = devices::query()->where('devices.name', 'like', '%' . $request->search . '%')
                ...
                ->orderBy('labs.uni_id')->withCount('reservations')
                ->withAvg('ratings as avg_service_quality', 'service_quality')
                ->get();
            ...
            $unidevices = $unidevs->join('uni_labs','uni_devices.lab_id','uni_labs.id')
                ...
                ->orderBy('uni_labs.uni_id')->withCount('reservations')->withAvg('ratings as avg_service_quality', 'service_quality')
                ->get();
            $faculties = \App\Models\fac_uni::all();
            // dd($devices,$unidevices);
