...
class DeviceRatingController extends Controller
{
    ...
    {
        $reservations = Reservation::with(['device', 'rating'])
            ->where('user_id', auth()->id())
            ->get()
            ->map(function ($item) {
                $item->central = 0; // from Reservation
                return $item;
            });

        $reservationsUni = ReservationUniLab::with(['device', 'rating'])
            ->where('user_id', auth()->id())
            ->get()
            ->map(function ($item) {
                $item->central = 1; // from ReservationUniLab
                return $item;
            });

        $reservations = $reservations->merge($reservationsUni);
    ...
    public function store(Request $request)
    {
        ...
        if ($request->central == 1) {
            ...
            $rules = [
            'reservation_id' => 'required|exists:reservation_uni_lab,id',
        ...
        $request->validate($rules);
            ...
            $reservation = ReservationUniLab::where('id', $request->reservation_id)
                ->where('user_id', auth()->id())
                ->where('confirmation', 'Confirmed')
                ->firstOrFail();

                DeviceRatingUniLab::create([
    ...
    {
        if ($request->central == 0) {
            $rating = DeviceRating::where('id', $rating)
                ->where('user_id', auth()->id())
                ->firstOrFail();
            // Check if the reservation exists and belongs to the authenticated user
            $rules = [
            ...
            $rating = DeviceRatingUniLab::where('id', $rating)
                ->where('user_id', auth()->id())
                ->firstOrFail();
            // Check if the reservation exists and belongs to the authenticated user
            $rules = [
