                            ...
                            <div class="card table-card">
                                <div class="card-header">
                                    ...
                                    <div class="card-header-right">
                                        <a class="" href="{{ route('DeptFac.create') }}">
...
{{--                                                <th>Coordinator</th>--}}
                                            ...
                                            @foreach ($departments as $dept)
                                                <tr>
                                                    ...
                                                    @role('admin')
                                                    <td>{{$universities[$dep_fac[$dept->id]->uni_id]->name}}</td>
                                                    <td>{{$faculties[$dep_fac[$dept->id]->fac_id]->name}}</td>
                                                    @endrole
                                                    @role('university')
{{--                                                    <td>{{\App\Models\fac_uni::where('uni_id',Auth()->user()->uni_id)->where('fac_id',$dep_fac[$dept->id]->fac_id)->pluck('name')->first()}}</td>--}}
                                                    <td>{{$faculties[$dep_fac[$dept->id]->fac_id]->name}}</td>
                                                    @endrole
                                                    <td class="text-right d-flex">
                                                        <a class="btn" href="{{ route('DeptFac.edit',$dept->id) }}">
                                                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" style="fill: rgba(68, 138, 255, 1);transform: ;msFilter:;"><path d="m18.988 2.012 3 3L19.701 7.3l-3-3zM8 16h3l7.287-7.287-3-3L8 13z"></path><path d="M19 19H8.158c-.026 0-.053.01-.079.01-.033 0-.066-.009-.1-.01H5V5h6.847l2-2H5c-1.103 0-2 .896-2 2v14c0 1.104.897 2 2 2h14a2 2 0 0 0 2-2v-8.668l-2 2V19z"></path></svg>
                                                        </a>
                                                    </td>
                                                    <td>
                                                        <form action="{{ route('DeptFac.destroy', $dept->id) }}" method="post" onSubmit="return confirm('Are You Sure To Delete This Department And Its content?')">
                                                            @csrf
                                                            @method("DELETE")
