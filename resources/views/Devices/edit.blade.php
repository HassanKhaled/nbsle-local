...
@section('loggedContent')
                        ...
                        <div class="col-md-6">
                            <div class="card">
                                ...
                                <div class="card-block">
                                    <form class="form-material " action="{{ route('DeviceLab.update',$dev->id) }}" method="POST" enctype="multipart/form-data" >
                                        @method('PUT')
                                        @csrf
                                        <div class="form-group form-primary">
                                            <input type="text" name="name" class="form-control" value="{{ old('name',$dev->name) }}" required="">
                                            <span class="form-bar"></span>
                                            <label class="float-label">Name <span class="text-c-red">*</span></label>
                                        </div>
                                        <div class="form-group form-primary">
                                            <input type="text" name="Arabicname" class="form-control" value="{{ old('Arabicname',$dev->Arabicname)}}" >
                                            <span class="form-bar"></span>
                                            <label class="float-label">اسم الجهاز باللغة العربية</label>
                                        </div>
                                        <div class="form-group form-primary">
                                            <input type="text" name="model" class="form-control" value="{{ old('model',$dev->model) }}" required="">
                                            <span class="form-bar"></span>
                                            <label class="float-label">Model <span class="text-c-red">*</span></label>
                                        </div>
                                        <div class="form-group form-primary">
                                            <input type="text" name="num_units" class="form-control" value="{{ old('num_units',$dev->num_units) }}" required="">
                                            <span class="form-bar"></span>
                                            <label class="float-label">Num of units <span class="text-c-red">*</span></label>
                                        </div>
                                        <div class="form-group form-primary">
                                            <input type="text" name="manufacturer" class="form-control" value="{{ old('manufacturer',$dev->manufacturer) }}">
                                            <span class="form-bar"></span>
                                        ...
                                        <div class="form-group form-primary" id="website">
                                            <input type="text" name="ArabicDescription" class="form-control" value="{{ old('ArabicDescription',$dev->ArabicDescription)}}">
                                            <span class="form-bar"></span>
                                            <label class="float-label">وصف الجهاز باللغة العربية</label>
                                        </div>
                                        <div class="form-group form-primary" id="price">
                                            <input type="text" name="price" class="form-control" value="{{ old('price',$dev->price)}}">
                                            <span class="form-bar"></span>
                                        ...
                                        <div class="container col-xs-12 col-sm-12 col-md-12 mb-2 morerows addRow" id="addRow">
                                            <div class="row form-group">
                                                ...
                                                <div class="col-md-3"><strong>الخدمة بالعربي</strong></div>
                                                <div class="col-md-3"><strong>Cost</strong></div>
                                                <div class="col-md-3"><strong>سعر/لكل</strong></div>
                                                @for($i = 0; $i <count($cost) ;$i++)
                                                    <div class="row form-group delete_add_more_item" id="delete_add_more_item">
                                                    <div class="col-md-3"><input type="text" name="services[]" class="form-control" value="{{ old('services.0',$services[$i]) }}" ></div>
                                                    <div class="col-md-3"><input type="text" name="servicesArabic[]" class="form-control" value="{{ old('servicesArabic.0',$servicesArabic[$i]) }}"></div>
                                                    <div class="col-md-3"><input type="text" name="cost[]" class="form-control" value="{{ old('cost.0',$cost[$i]) }}"></div>
                                                    <div class="col-md-3"><input type="text" name="costArabic[]" class="form-control" value="{{ old('costArabic.0',$costArabic[$i])}}"> </div>
                                                    @if($i!=0)<i class="col-md-2 removeaddmore" style="cursor:pointer;color:red;"> Remove </i>@endif
                                                    </div>
                                                @endfor
                                            </div>
                                        </div>
                                        <div class="form-group form-primary"><a id="addMore" class="btn btn-primary text-white btn-sm">Add More </a></div>
                                        @role('admin')
                                        <div class="form-group form-primary">
                                        ...
                                        <div class="form-group form-primary">
                                            ...
                                            <select name="fac_id" class="facultyselect" id="facultyselect" onchange="runDept(this.value)">
                                                ...
                                                @foreach($facultys as $fac)
                                                    <option value="{{$fac->fac_id}}" {{old('fac_id',$labs[$dev->lab_id]->fac_id)==$fac->fac_id?'selected':''}}> {{$fac->name}} </option>
                                                @endforeach
                                            </select>
                                        </div>
                                        @endrole
                                            ...
                                            <div class="form-group form-primary">
                                                ...
                                                <select name="dept_id" class="departmentselect" id="departmentselect" onchange="runLab(this.value)">
                                                    ...
                                                    @foreach($departments as $dept)
                                                        <option value="{{$dept->id}}" {{old('dept_id',$labs[$dev->lab_id]->dept_id)==$dept->id?'selected':''}}> {{$dept->name}} </option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        @endcan
                                        <div class="form-group form-primary">
                                            ...
                                            <select id="lab" name="lab_id" >
                                                ...
                                                @foreach($labs as $lab)
                                                    <option value="{{$lab->id}}" {{$lab->id == old('lab_id',$dev->lab_id) ?'selected':''}}>{{$lab->name}}
                                                    </option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="form-group form-primary">
                                            ...
                                            <select id="lab" name="state" class="stateselect">
                                                <option value="available" {{old('state',$dev->state)!='maintenance'?'selected':''}}>Available</option>
                                                <option value="maintenance" {{old('state',$dev->state)=='maintenance'?'selected':''}}>Under Maintenance</option>
                                            </select>
                                        </div>
                                        <div class="form-group form-primary" id="image">
                                            <label>Image</label>
                                            <br><img src="{{asset($dev->ImagePath)}}" style="max-width: 100px"><br><br>
                                            <input type="file" id="pic" name="ImagePath" accept="image/*" class="form-control" style="height:50px">
                                            <span class="form-bar"></span>
                                        </div>
                                        <div class="form-group form-primary" id="website">
                                            <input type="text" name="AdditionalInfo" class="form-control" value="{{old('AdditionalInfo',$dev->AdditionalInfo)}}">
                                            <span class="form-bar"></span>
                                            <label class="float-label">Additional Info</label>
                                        </div>
                                        <div class="form-group form-primary" id="website">
                                            <input type="text" name="ArabicAddInfo" class="form-control" value="{{old('ArabicAddInfo',$dev->ArabicAddInfo)}}">
                                            <span class="form-bar"></span>
    ...
    <script id="document-template" type="text/x-handlebars-template">
        <div class="row form-group delete_add_more_item" id="delete_add_more_item">
            ...
            <div class="col-md-3"><input type="text" name="servicesArabic[]" class="services form-control" placeholder="الخدمة"></div>
    ...
    <script>
        ...
        function runLab(selected_dept){
            $('#lab').find('option:not(:first)').remove();
            var labs = @json($labs);
            var selected_fac = $('#facultyselect').val();
            $.each(labs, function(i, val) {
                console.log(selected_fac);
                if (val.dept_id==selected_dept && val.fac_id == selected_fac ){
                    $('#lab').append($(document.createElement('option')).prop({
                        value: val.id,
                        text: val.name
                    }))
