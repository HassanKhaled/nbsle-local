                                                    ...
                                                    <td>
                                                        <div class="d-inline-block align-middle">
                                                            <div class="d-inline-block">
                                                                <h6>{{$lab->name==null?$lab->Arabicname:$lab->name}}</h6>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    @role('admin')
                                                    <td >{{\App\Models\universitys::where('id',$lab->uni_id)->first()->name}}</td>
                                                    <td>{{\App\Models\facultys::where('id',$lab->fac_id)->pluck('name')->first()}}</td>
                                                    <td>{{$lab->dept_id==null?'':\App\Models\departments::where('id',$lab->dept_id)->pluck('name')->first()}}</td>
                                                    @endrole
                                                    @role('university')
                                                    <!--<td>{{\App\Models\facultys::where('id',$lab->fac_id)->pluck('name')->first()}}</td> -->
                                                    <td>{{\App\Models\fac_uni::where('uni_id',auth()->user()->uni_id)->where('fac_id',$lab->fac_id)->pluck('name')->first()}}</td>
                                                    <td>{{$lab->dept_id==null?'':\App\Models\departments::where('id',$lab->dept_id)->pluck('name')->first()}}</td>
                                                    @endrole
                                                    @role('faculty')<td>{{$lab->dept_id==null?'':\App\Models\departments::where('id',$lab->dept_id)->pluck('name')->first()}}</td>@endrole
                                                    <td class="text-right d-flex">
                                                        <a class="btn" href="{{ route('Lab.edit',$lab->id) }}">
                                                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" style="fill: rgba(68, 138, 255, 1);transform: ;msFilter:;"><path d="m18.988 2.012 3 3L19.701 7.3l-3-3zM8 16h3l7.287-7.287-3-3L8 13z"></path><path d="M19 19H8.158c-.026 0-.053.01-.079.01-.033 0-.066-.009-.1-.01H5V5h6.847l2-2H5c-1.103 0-2 .896-2 2v14c0 1.104.897 2 2 2h14a2 2 0 0 0 2-2v-8.668l-2 2V19z"></path></svg>
                                                        </a>
                                                    </td>
                                                    <td>
                                                        <form action="{{ route('Lab.destroy', $lab->id) }}" method="post" onSubmit="return confirm('Are You Sure To Delete This Lab And Its Devices?')">
                                                            @csrf
                                                            @method("DELETE")
