                        ...
                        <div class="col-xl-12 col-md-12">
                            <div class="card table-card">
                                <div class="card-header">
                                    <h5>Central Devices</h5>
                                    <div class="card-header-right">
                                        <a class="" href="{{ route('UniDevice.create') }}">
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-hover" id="myTable">
                                            ...
                                            @foreach ($devices as $device)
                                                <tr>
                                                    <td><img src="{{asset($device->ImagePath)}}" style="max-width: 200px"></td>
                                                    <td>
                                                        <div class="d-inline-block align-middle">
                                                            <div class="d-inline-block">
                                                                <h6>{{$device->name==null?$device->Arabicname:$device->name}}</h6>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    <td>{{$device->lab->name}}</td>
                                                    <td>{{$universities[$device->lab->uni_id]->name}}</td>
                                                    <td class="text-right d-flex">
                                                        <a class="btn" href="{{ route('UniDevice.edit',$device->id) }}">
                                                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" style="fill: rgba(68, 138, 255, 1);transform: ;msFilter:;"><path d="m18.988 2.012 3 3L19.701 7.3l-3-3zM8 16h3l7.287-7.287-3-3L8 13z"></path><path d="M19 19H8.158c-.026 0-.053.01-.079.01-.033 0-.066-.009-.1-.01H5V5h6.847l2-2H5c-1.103 0-2 .896-2 2v14c0 1.104.897 2 2 2h14a2 2 0 0 0 2-2v-8.668l-2 2V19z"></path></svg>
                                                        </a>
                                                    </td>
                                                    <td>
                                                        <form action="{{ route('UniDevice.destroy', $device->id) }}" method="post" onSubmit="return confirm('Are You Sure To Delete This Device?')">
                                                            @csrf
                                                            @method("DELETE")
