                        ...
                        <div class="col-md-6">
                            <div class="card">
                                ...
                                <div class="card-block">
                                    <form class="form-material " action="{{ route('UniLab.update',$lab->id) }}" method="POST">
                                        @csrf
                                        @method('PUT')
                                        ...
                                        <div class="form-group form-primary">
                                            <input type="text" name="location" class="form-control" value="{{ old('location',$lab->location) }}" required="">
                                            <span class="form-bar"></span>
                                        ...
                                        <div class="form-group form-primary">
                                            ...
                                            <select id="uni" name="uni_id" >
                                                ...
                                                @foreach($universities as $uni)
                                                    <option value="{{old('uni_id',$uni->id)}}" {{$lab->uni_id == $uni->id? 'selected':''}}> {{$uni->name}} </option>
                                                @endforeach
                                            </select>
                                        </div>
                                        @endrole
                                        <div class="container col-xs-12 col-sm-12 col-md-12 mb-2 morerows addRow" id="addRow">
                                            <div class="row form-group">
                                                @foreach($coors as $coor)
                                                    ...
                                                    <div class="col-md-3"><input type="text" name="names[]" class="form-control" value="{{$coor->name}}" placeholder="{{$coor->name}}"></div>
                                                    <div class="col-md-3"><input type="text" name="emails[]" class="form-control" value="{{$coor->email}}" placeholder="{{$coor->email}}"> </div>
                                                    <div class="col-md-3"><input type="text" name="phones[]" class="form-control" value="{{$coor->telephone}}" placeholder="{{$coor->telephone}}"> </div>
                                                    <div class="col-md-1"><input type="checkbox" name="staffs[]" class="cost form-control" {{$coor->staff==1? 'checked':''}} value="1"></div>
                                                    <div class="col-md-1"><input type="checkbox" name="staffs[]" class="cost form-control" {{$coor->staff==0? 'checked':''}} value="0"></div>
                                                @endforeach
                                            </div>
                                        </div>
                                                <div class="col-md-2"><i id="addMore" class="btn btn-primary btn-sm">Add another</i></div>
