...
<div class="container-fluid py-5 mt-5">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4 mt-5">
        ...
        <a href="{{ route('uniworkshop', auth()->user()->uni_id) }}" class="btn btn-sm btn-primary">
          add workshop      
        </a>
    </div>
    ...
    {{-- Filters --}}
    <form method="GET" action="{{ route('uniworkshops.index') }}" class="card shadow-sm mb-4 p-3">
            ...
            <div class="col-md-3">
                ...
                <select name="university" id="filter-university" class="form-control">
                    ...
                    @foreach($universities as $uni)
                        <option value="{{ $uni->id }}" {{ request('university') == auth()->user()->uni_id? 'selected' : '' }}>
                            {{ $uni->name }}
                        </option>
                    @endforeach
                </select>
